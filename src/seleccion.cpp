...
bool Seleccion( int x, int y, Escena * escena, ContextoVis & cv_dib )
{
   ...
   if(id == 0)
   {
      cout << "id : 0. No hay objeto seleccionable" << endl;
      return false;
   }
   ...
   Tupla3f centro;
   if(objeto->buscarObjeto(id, MAT_Ident(), &objeto_buscado, centro))
   {
      camara->mirarHacia(centro);
      cout << "Objeto seleccionado: " << objeto_buscado->leerNombre() << endl;
