...
Textura::Textura( const std::string & nombreArchivoJPG )
{
   ...
   // .....
    const char * nombreArchivoJPGChar = nombreArchivoJPG.c_str();
    imagen = LeerArchivoJPEG(nombreArchivoJPGChar, ancho, alto);
...
void Textura::enviar()
{
   ...
   // .......
    ...
    //Creación del identificador de textura
...
void Material::activar( Cauce & cauce )
{
   // COMPLETAR: práctica 4: activar un material
    ...
    Tupla3f ambiente = {k_amb, k_amb, k_amb};
    Tupla3f difusa = {k_dif, k_dif, k_dif};
    Tupla3f especular = {k_pse, k_pse, k_pse};
    
    cauce.fijarParamsMIL(ambiente, difusa, especular, exp_pse );
